...
import { 
    ...
    Settings, 
    LogOut,
    Image // Nuova icona per la galleria
} from "lucide-react";
...
const items = [
    ...
    { to: "/admin/gallery", label: "Media & Galleria", icon: Image, testid: "nav-gallery" }, // Voce integrata
            ...
            <aside className="w-64 border-r border-lake-border bg-lake-cream flex flex-col" data-testid="admin-sidebar">
                ...
                <nav className="flex-1 p-3">
                    {items.map((it) => (
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `flex items-center gap-3 px-4 py-2.5 my-1 rounded-sm text-sm transition-all ${
                                    isActive 
                                    ? "bg-lake-blue text-white shadow-sm" 
                                    : "text-lake-ink hover:bg-white"
                                }`
                            }
                        >
                ...
                <div className="p-4 border-t border-lake-border">
                    ...
                    <button 
                        ...
                        className="mt-2 flex items-center gap-2 text-sm text-lake-ink hover:text-red-500 transition-colors"
                    >
                        <LogOut size={14} /> Esci
                    </button>
                </div>
